...
// ---- Config Supabase (pondrás las variables en Railway) ----
...
// Sube archivo local a Supabase y devuelve URL (pública o firmada)
async function uploadToSupabase(localPath, destKey, contentType) {
  ...
  const { error } = await supabase.storage.from(SUPABASE_BUCKET)
    .upload(destKey, fileBuffer, { contentType, upsert: true });
